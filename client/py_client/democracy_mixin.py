...
class _DemocracyMixin:
    def submit_set_inactivity_timeout_proposal(self, account, inactivity_timeout, cid=None, pay_fees_in_cc=False):
        ret = self.run_cli_command(["democracy", "submit-set-inactivity-timeout", account, str(inactivity_timeout)], cid,
                                   pay_fees_in_cc)
        return ret.stdout.decode("utf-8").strip()

    def submit_update_nominal_income_proposal(self, account, new_income, cid=None, pay_fees_in_cc=False):
        ret = self.run_cli_command(["democracy", "submit-update-nominal-income", account, str(new_income)], cid,
                                   pay_fees_in_cc)
        return ret.stdout.decode("utf-8").strip()
    ...
    def submit_update_demurrage_proposal(self, account, demurrage_halving_blocks, cid=None, pay_fees_in_cc=False):
        ret = self.run_cli_command(
            ["democracy", "submit-update-demurrage", account, str(demurrage_halving_blocks)], cid, pay_fees_in_cc)
        return ret.stdout.decode("utf-8").strip()

    def submit_petition(self, account, demand, cid=None, pay_fees_in_cc=False):
        ret = self.run_cli_command(["democracy", "submit-petition", account, demand], cid, pay_fees_in_cc)
        return ret.stdout.decode("utf-8").strip()

    def submit_spend_native_proposal(self, account, to, amount, pay_fees_in_cc=False):
        ret = self.run_cli_command(["democracy", "submit-spend-native", account, to, str(amount)],
    ...
    def update_proposal_state(self, account, proposal_id, cid=None, pay_fees_in_cc=False):
        ret = self.run_cli_command(["democracy", "update-proposal-state", account, str(proposal_id)], cid, pay_fees_in_cc)
        return ret.stdout.decode("utf-8").strip()

    def list_proposals(self):
        ret = self.run_cli_command(["democracy", "list-proposals"])
        return ret.stdout.decode("utf-8").strip()
    ...
    def list_enactment_queue(self):
        ret = self.run_cli_command(["democracy", "list-enactments"])
        return ret.stdout.decode("utf-8").strip()
